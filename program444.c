...
void Insert(PPNODE Root, int No)
{
    ...
    else    // Tree contains atleast one node
    {
        ...
        while(1)    // while(1) means unconditional loop
        {
            if(temp->data == No)
            {
                printf("Unable to insert node as elements is already present\n");
                free(newn);
                break;
            }
...
int main()
{
    ...
    printf("There are %d Leafnodes in BST\n", iRet);

    iRet = CountParent(Head);

    printf("There are %d Parent nodes in BST\n", iRet);

    iRet = CountAll(Head);

    printf("There are %d nodes in BST\n", iRet);
